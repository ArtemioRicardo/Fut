...
def match_filter(mdata, team, match, competicion):
    #print(type(match))
    if (competicion!='All')&(team!='All')&(match!='All'):
        match_data = mdata[(mdata['competition.competition_name']==competicion)&((mdata['home_team.home_team_name'] == team) | (mdata['away_team.away_team_name'] == team)) & (mdata['match_id']==match)]
    elif (competicion!='All')&(team!='All')&(match=='All'):
        match_data = mdata[(mdata['competition.competition_name']==competicion)&((mdata['home_team.home_team_name'] == team) | (mdata['away_team.away_team_name'] == team))]
    elif (competicion!='All')&(team=='All')&(match!='All'):
        match_data = mdata[(mdata['competition.competition_name']==competicion)&(mdata['match_id']==match)]
    elif (competicion!='All')&(team=='All')&(match=='All'):
        match_data = mdata[(mdata['competition.competition_name']==competicion)]
    elif (competicion=='All')&(team!='All')&(match!='All'):
...
Total_player = list(filter(lambda x: x is not None, Total_player))
...
Total_outcome = list(filter(lambda x: x is not None, Total_outcome))
optionslist_outcome=[{'label': i, 'value': i} for i in Total_outcome]
optionslist_outcome.insert(0,{'label': 'All', 'value': 'All'})

Total_competition=Match['competition.competition_name'].unique()
optionslist_competition=[{'label': i, 'value': i} for i in Total_competition]
optionslist_competition.insert(0,{'label': 'All', 'value': 'All'})
...
sidebar = html.Div(
    [
        ...
        html.Hr(),
        html.P(children ="Danger Shot Only: ", style = {"color": Texclaro}),
        dcc.RadioItems( id='Danger_shot', options=[{'label':'Si', 'value':'Yes'}, {'label':'No', 'value':'No'},], value='Yes', style ={"color":Texclaro}, labelStyle={'display':'inline-block'}),
        #html.Hr(),
        html.P(
            children = "Competicion: ",
            style = {"color": Texclaro}
            ),
        dcc.Dropdown(
            id = "dropdown_competicion",
            multi = False,
            searchable = True,
            value = "All",
            clearable=False,
            disabled= False,
            options=optionslist_competition,
...
@app.callback(
    ...
    Input(component_id = "dropdown_competicion",	component_property = "value")])
def update_KPI_CARDS_GLOBAL(team, match_, competicion):
    match_fil=match_filter(Match, team, match_, competicion)
    ...
    #print(match_fil.columns.values)
...
@app.callback(
    ...
    Output(	component_id = "temporada",	component_property = "figure")],
    [Input(component_id = "dropdown_team",	component_property = "value"),
...
def Match_pie_chart_global(team, type, match_, possession, player, outcome, competicion):
    match_filt=match_filter(Match, team, match_, competicion)
    data_pglo=match_filt.groupby(["stadium.country.name"]).size().nlargest(5).reset_index(name='counts')# Calculate values 
    temoprada_bar=match_filt.groupby(["competition.competition_name"]).size().nlargest(8).reset_index(name='counts')# Calculate values 
    fig = px.bar(data_pglo, x='counts', y='stadium.country.name', hover_data=['stadium.country.name'], color="stadium.country.name", color_discrete_map = color_discrete, height= 250)
    fig1 = px.bar(temoprada_bar, x='counts', y='competition.competition_name')
    ...
    fig1.update_layout(title = f"Competicion:", yaxis={'categoryorder':'total ascending'}),
    fig.update_layout(showlegend=False)
    return fig, fig1

@app.callback(
	[Output(component_id = "Field_chart", component_property = "figure"),
    ...
    Input(component_id = "dropdown_shot_outcome_name",  component_property = "value"),
    Input(component_id = "dropdown_competicion",	component_property = "value"),
    Input(component_id = "Danger_shot",  component_property = "value")]    
)
def events_Field_chart(team, typo, match, possession, player, outcome, competicion, danger):
    if danger=='Yes':
        shot_dan=events[(events['shot.dangerous']==1)] 
    else:        
        shot_dan=events
    match_filt=match_filter(Match, team, match, competicion)
    data_assignee=datafilter_report(shot_dan, team, typo, match, possession, player, outcome, 'All')
    data_assignee = pd.merge(data_assignee, match_filt, on=["match_id"])
    ...
    datos_barglo2=data_assignee.groupby(["type.name"], dropna=True).size().nlargest(10).reset_index(name='counts')
    ...
    if (typo == 'Shot'):
        fig = px.scatter(data_assignee, x = 'loc_x',y='loc_y',  size='shot.statsbomb_xg', color="shot.dangerous", color_discrete_map = color_discrete, 
                            hover_name="player.name", hover_data=["match_id", "pase_nombre"])
    else:
        fig = px.scatter(data_assignee, x = 'loc_x',y='loc_y', color="type.name", color_discrete_map = color_discrete, 
                            hover_name="player.name", hover_data=["match_id",'team.name',  'home_team.home_team_name', 'away_team.away_team_name'])
    
    import base64
    #set a local image as a background
    image_filename = 'Football_field.svg.png'
    ...
    if datos_barglo.empty:
        ...
        if (player != 'All')&(typo == 'Shot'):
            fig1 = px.scatter(data_assignee, x = 'loc_x_pass',y='loc_y_pass',  size='shot.statsbomb_xg', color="shot.dangerous", color_discrete_map = color_discrete, 
                                hover_name="player.name", hover_data=["match_id", "pase_nombre"])
            import base64
            #set a local image as a background
            image_filename = 'Football_field.svg.png'
...
def events_gol(team, typo, match, possession, player, outcome, competicion, danger):
    if danger=='Yes':
        shot_dan=events[(events['shot.dangerous']==1)] 
    else:        
        shot_dan=events 
    ...
    fig = go.Figure()   
    if (outcome != 'All'):
        fig.add_trace(go.Scatter(x = gol['gol_y'], y = gol['gol_z'], marker = {'color': 'blue', 'size': 10}, mode="markers",  
        hovertext=gol['shot.body_part.name'], hoverinfo="text"))
#        fig = px.scatter(gol, x = 'gol_y',y='gol_z', color="shot.dangerous", color_discrete_map = color_discrete)
    else:
        fig.add_trace(go.Scatter(x = gol['gol_y'], y = gol['gol_z'], marker = {'color': color, 'colorscale': colorscale, 'size': 10}, mode="markers", 
        hovertext=gol["shot.outcome.name"], hoverinfo="text"))
#        fig = px.scatter(gol, x = 'gol_y',y='gol_z', color="shot.dangerous", color_discrete_map = color_discrete)
...
def events_KPI(team, typo, match, possession, player, outcome, competicion, danger):
    if danger=='Yes':
        shot_dan=events[(events['shot.dangerous']==1)] 
    else:        
        shot_dan=events 
...
@app.callback(
    ...
    Input(component_id = "dropdown_competicion",	component_property = "value"),
    Input(component_id = "Danger_shot",  component_property = "value")])
def update_data_jira(team, typo, match, possession, player, outcome, competicion, danger):
    if danger=='Yes':
        shot_dan=events[(events['shot.dangerous']==1)] 
    else:        
        shot_dan=events 
    ...
    data_assignee=datafilter_report(shot_dan, team, typo, match, possession, player, outcome, 'All')
    data_assignee = pd.merge(data_assignee, match_filt, on=["match_id"])
    cross=pd.crosstab(data_assignee['player.name'], data_assignee['shot.outcome.name'], margins=True)#.reindex(columns=a, index=a, fill_value=0)
